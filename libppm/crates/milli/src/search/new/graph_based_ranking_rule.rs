...
impl<G: RankingRuleGraphTrait> GraphBasedRankingRule<G> {
    fn with_term_removal(remove_terms: bool) -> Self {
        Self { remove_terms, state: None }
    }
}
...
impl<'ctx, G: RankingRuleGraphTrait> RankingRule<'ctx, QueryGraph> for GraphBasedRankingRule<G> {
    ...
    fn start_iteration(
        ...
        let removal_cost = if self.remove_terms {
            let removal_order = query_graph.removal_order_for_terms_matching_strategy_last(ctx);
            let mut forbidden_nodes = SmallBitmap::for_interned_values_in(&query_graph.nodes);
    ...
    #[tracing::instrument(level = "trace", skip_all, target = "search::graph_based")]
    fn end_iteration(&mut self, _ctx: &mut SearchContext<'ctx>) {
        self.state = None;
    }
}
