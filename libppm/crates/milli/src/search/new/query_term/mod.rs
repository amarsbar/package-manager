...
impl QueryTermSubset {
    ...
    pub fn all_phrases(&self, ctx: &mut SearchContext<'_>) -> Result<BTreeSet<Interned<Phrase>>> {
        ...
        let ZeroTypoTerm { phrase, exact: _, prefix_of: _, use_prefix_db: _ } =
            &original.zero_typo;
...
impl ZeroTypoTerm {
    fn is_empty(&self) -> bool {
        let ZeroTypoTerm { phrase, exact: zero_typo, prefix_of, use_prefix_db } = self;
        phrase.is_none()
            && zero_typo.is_none()
            && prefix_of.is_empty()
...
impl QueryTerm {
    ...
    pub fn all_computed_derivations(&self) -> (Vec<Interned<String>>, Vec<Interned<Phrase>>) {
        ...
        let ZeroTypoTerm { phrase, exact: zero_typo, prefix_of, use_prefix_db: _ } =
            &self.zero_typo;
