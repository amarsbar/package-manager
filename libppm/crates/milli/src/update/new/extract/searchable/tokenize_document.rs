...
use crate::{FieldId, InternalError, Result, MAX_WORD_LENGTH};
...
impl DocumentTokenizer<'_> {
    ...
    fn tokenize_field(
        ...
        let tokens = match value {
            ...
            Value::String(text) => self.tokenizer.tokenize(text.as_str()),
            _ => return Ok(()),
        };
...
pub fn tokenizer_builder() -> TokenizerBuilder<'static, &'static [u8]> {
...
mod test {
    ...
    fn test_tokenize_document() {
        ...
        let document_tokenizer =
            DocumentTokenizer { tokenizer: &tb.build(), max_positions_per_attributes: 1000 };
